...
def checkConfig():
	db = dbconfig()
	cursor = db.cursor()
	query = "SELECT SCHEMA_NAME FROM INFORMATION_SCHEMA.SCHEMATA  WHERE SCHEMA_NAME = 'passwordManager'"
...
def make():
	...
	try:
		...
		printc("[red][!] An error occurred while trying to create db. Check if database with name 'passwordManager' already exists - if it does, delete it and try again.")
	...
	query = "CREATE TABLE passwordManager.entries (sitename TEXT NOT NULL, siteurl TEXT NOT NULL, email TEXT, username TEXT, password TEXT NOT NULL)"
	res = cursor.execute(query)
