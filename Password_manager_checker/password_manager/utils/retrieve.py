...
def computeMasterKey(masterPassword,deviceSecret):
	password = masterPassword.encode()
	salt = deviceSecret.encode()
	key = PBKDF2(password, salt, 32, count=1000000, hmac_hash_module=SHA512)
	return key

def retrieveEntries(masterPassword, deviceSecret, search, decryptPassword = False):
	db = dbconfig()
	cursor = db.cursor()
	...
	if decryptPassword and len(results)==1:
		# Compute master key
		mk = computeMasterKey(masterPassword,deviceSecret)
